...
var _ = Describe("DroppedCounter", func() {
	...
	It("ignores congested IPs for dopplers when the dropped count is zero", func() {
		counter, byteWriter := newCounterAndMockWriter()
		counter.DropCongested(0, "testDoppler")
		counter.Drop(10)
		Expect(counter.Dropped()).To(BeEquivalentTo(10))
		...
		actualLog := actualEnvelopes[1]
		Expect(actualLog.EventType).To(Equal(events.Envelope_LogMessage.Enum()))
		Expect(actualLog.LogMessage.Message).ToNot(ContainSubstring("Congested dopplers"))
	})

	It("includes IP address for a congested Doppler", func() {
		counter, byteWriter := newCounterAndMockWriter()
	...
	It("includes IP address for multiple congested Dopplers", func() {
		counter, byteWriter := newCounterAndMockWriter()
		counter.DropCongested(10, "testDoppler")
		counter.DropCongested(10, "anotherTestDoppler")
		Expect(counter.Dropped()).To(BeEquivalentTo(20))
