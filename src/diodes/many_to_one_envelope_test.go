...
var _ = Describe("ManyToOneEnvelope", func() {
	var (
		d    *diodes.ManyToOneEnvelope
	...
	Describe("Next()", func() {
		BeforeEach(func() {
			...
			d = diodes.NewManyToOneEnvelope(5, mockAlerter)

			data = &events.Envelope{Origin: proto.String("some-origin")}
			d.Set(data)
		})
		...
		Context("multiple data slices", func() {
			var (
				secondData *events.Envelope
			)

			BeforeEach(func() {
				secondData = &events.Envelope{Origin: proto.String("some-other-origin")}
				d.Set(secondData)
			})
			...
			Context("reads exceed writes", func() {
				...
				BeforeEach(func() {
					rxCh = make(chan *events.Envelope, 100)
					for i := 0; i < 2; i++ {
						d.Next()
					}
					wg.Add(1)
