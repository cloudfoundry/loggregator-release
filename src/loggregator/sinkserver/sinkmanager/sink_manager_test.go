...
var _ = Describe("SinkManager", func() {
	...
	var oldNewDumpSink func(string, uint32, *gosteno.Logger, time.Duration) *dump.DumpSink
	var calledInactivityDuration chan time.Duration

	BeforeEach(func() {
		oldNewDumpSink = sinkmanager.NewDumpSink
		calledInactivityDuration = make(chan time.Duration, 1024)
		sinkmanager.NewDumpSink = func(appId string, bufferSize uint32, givenLogger *gosteno.Logger, inactivityDuration time.Duration) *dump.DumpSink {
			calledInactivityDuration <- inactivityDuration
			return dump.NewDumpSink(appId, bufferSize, givenLogger, inactivityDuration)
		}

		sinkManager, appServicesChan = sinkmanager.NewSinkManager(1, true, blackListManager, loggertesthelper.Logger(), 42*time.Millisecond)
	...
	AfterEach(func() {
		...
		sinkmanager.NewDumpSink = oldNewDumpSink
	})

	Describe("SendSyslogErrorToLoggregator", func() {
		It("listens and broadcast error messages", func() {
	...
	Describe("SendTo", func() {
		...
		Context("when no dump sinks are available", func() {
			It("sets the proper timeout on the new dump sink", func() {
				expectedMessageString := "Some Data"
				expectedMessage := NewMessage(expectedMessageString, "myApp")
				go sinkManager.SendTo("myApp1", expectedMessage)

				Eventually(calledInactivityDuration).Should(Receive(Equal(42 * time.Millisecond)))
			})
		})
