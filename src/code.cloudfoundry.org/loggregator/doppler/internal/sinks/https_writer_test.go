...
var _ = Describe("HttpsWriter", func() {
	...
	Context("With an HTTPS Sink", func() {
		...
		It("requires TLS Version 1.2", func() {
			...
			w, err := sinks.NewHttpsWriter(outputUrl, "appId", "org-name.space-name.app-name.1", true, dialer, timeout)
		...
		It("HTTP POSTs each log message to the HTTPS syslog endpoint", func() {
			...
			w, _ := sinks.NewHttpsWriter(outputUrl, "appId", "org-name.space-name.app-name.1", true, dialer, timeout)
			err := w.Connect()
		...
		It("returns an error for syslog-tls scheme", func() {
			...
			_, err := sinks.NewHttpsWriter(outputUrl, "appId", "org-name.space-name.app-name.1", false, dialer, timeout)
			Expect(err).To(HaveOccurred())
		})
		...
		It("returns an error when the provided dialer is nil", func() {
			...
			_, err := sinks.NewHttpsWriter(outputURL, "appId", "org-name.space-name.app-name.1", false, nil, timeout)
			Expect(err).To(HaveOccurred())
...
func (c *concurrentWriteRequestSimulator) concurrentWriteRequests(count int, writer sinks.Writer) {
	wg := &sync.WaitGroup{}
	wg.Add(count)
