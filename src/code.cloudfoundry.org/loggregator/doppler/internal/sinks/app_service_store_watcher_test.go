...
var _ = Describe("AppServiceStoreWatcher", func() {
	var watcher *sinks.AppServiceStoreWatcher
	var watcherRunComplete sync.WaitGroup

	var runWatcher func()

	var adapter storeadapter.StoreAdapter
	var outAddChan <-chan sinks.AppService
	var outRemoveChan <-chan sinks.AppService

	var app1Service1 sinks.ServiceInfo
	var app1Service2 sinks.ServiceInfo
	var app2Service1 sinks.ServiceInfo

	BeforeEach(func() {
		app1Service1 = sinks.NewServiceInfo(APP1_ID, "syslog://example.com:12345", "org.space.app-one.1")
		app1Service2 = sinks.NewServiceInfo(APP1_ID, "syslog://example.com:12346", "org.space.app-one.1")
		app2Service1 = sinks.NewServiceInfo(APP2_ID, "syslog://example.com:12345", "org.space.app-two.1")
		...
		c := sinks.NewAppServiceCache()
		watcher, outAddChan, outRemoveChan = sinks.NewAppServiceStoreWatcher(adapter, c)
	...
	Context("when there is an error", func() {
		...
		BeforeEach(func() {
			adapter = newFSA()
			watcher, _, _ := sinks.NewAppServiceStoreWatcher(adapter, sinks.NewAppServiceCache())
	...
	Describe("when the store has data and watcher is bootstrapped", func() {
		...
		Context("when there is new data in the store", func() {
			Context("when an existing app has a new service through a create operation", func() {
				It("adds that service to the outgoing add channel", func() {
					app2Service2 := sinks.NewServiceInfo(APP2_ID, "syslog://new.example.com:12345", "org.space.app-two.1")
			...
			Context("when a new app appears", func() {
				It("adds that app and its services to the outgoing add channel", func() {
					app3Service1 := sinks.NewServiceInfo(APP3_ID, "syslog://app3.example.com:12345", "org.space.app-three.1")
					app3Service2 := sinks.NewServiceInfo(APP3_ID, "syslog://app3.example.com:12346", "org.space.app-three.1")
		...
		Context("when an existing app service expires", func() {
			It("removes the app service from the cache", func() {
				app2Service2 := sinks.NewServiceInfo(APP2_ID, "syslog://foo/a", "org.space.app-two.1")
				adapter.Get(key(app2Service2))
				adapter.Create(buildNode(app2Service2))
				var appService sinks.AppService
