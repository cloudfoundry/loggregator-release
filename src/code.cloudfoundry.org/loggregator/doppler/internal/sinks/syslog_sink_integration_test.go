	...
	Describe("HTTPS writer", func() {
		Context("with non-200 return codes", func() {
			PIt("backs off", func() {
				...
				dialer := &net.Dialer{}
				httpsWriter, err := sinks.NewHttpsWriter(url, appId, "loggregator", true, dialer, 0)
				Expect(err).ToNot(HaveOccurred())

				errorHandler := func(errorMsg, appId string) {}

				syslogSink := sinks.NewSyslogSink(appId, url, bufferSize, httpsWriter, errorHandler, "dropsonde-origin")
