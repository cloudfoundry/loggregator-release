...
var _ = Describe("SyslogSink", func() {
	...
	JustBeforeEach(func() {
		...
		syslogSink = sinks.NewSyslogSink("appId", drainURL, bufferSize, sysLogger, errorHandler, "dropsonde-origin")
	})

	It("returns an identifier for a URL", func() {
		u, _ := url.Parse("syslog://example.com/some-path?foo=bar")
		identifier := sinks.IdentifierFromURL(u)
	...
	Context("when remote syslog server is up", func() {
		JustBeforeEach(func() {
			go func(*sinks.SyslogSink, chan bool) {
				syslogSink.Run(inputChan)
				close(syslogSinkRunFinished)
