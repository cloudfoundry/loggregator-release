...
var _ = Describe("TLSWriter", func() {
	...
	Describe("New", func() {
		It("returns an error for syslog scheme", func() {
			...
			_, err := sinks.NewTlsWriter(outputURL, "appId", "hostname", false, dialer, ioTimeout)
		...
		It("returns an error if the provided dialer is nil", func() {
			...
			_, err := sinks.NewTlsWriter(outputURL, "appId", "hostname", false, nil, ioTimeout)
			Expect(err).To(HaveOccurred())
		...
		It("requires TLS Version 1.2", func() {
			outputURL, _ := url.Parse("syslog-tls://localhost")
			w, err := sinks.NewTlsWriter(outputURL, "appId", "hostname", false, dialer, ioTimeout)
	...
	Describe("Write", func() {
		...
		JustBeforeEach(func() {
			...
			syslogWriter, err = sinks.NewTlsWriter(outputURL, "appId", "hostname", skipCertVerify, dialer, ioTimeout)
