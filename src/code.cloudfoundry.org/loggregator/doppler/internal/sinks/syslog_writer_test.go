...
var _ = Describe("SyslogWriter", func() {
	...
	BeforeEach(func() {
		...
		sysLogWriter, _ = sinks.NewSyslogWriter(
			outputURL,
			"appId",
	...
	It("returns an error when the provided dialer is nil", func() {
		...
		_, err := sinks.NewSyslogWriter(outputURL, "appId", "hostname", nil, 0)
		Expect(err).To(HaveOccurred())
	...
	It("returns an error for syslog-tls scheme", func() {
		...
		_, err := sinks.NewSyslogWriter(outputURL, "appId", "hostname", dialer, 0)
		Expect(err).To(HaveOccurred())
	})
	...
	Describe("network timeouts", func() {
		...
		JustBeforeEach(func() {
			...
			sysLogWriter, err = sinks.NewSyslogWriter(url, "appId", "hostname", dialer, writeTimeout)
			Expect(err).NotTo(HaveOccurred())
