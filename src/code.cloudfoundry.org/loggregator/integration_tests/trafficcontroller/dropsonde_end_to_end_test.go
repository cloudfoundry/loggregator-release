...
var _ = Describe("TrafficController for dropsonde messages", func() {
	...
	Context("Recent", func() {
		...
		BeforeEach(func() {
			...
			for i := 0; i < 5; i++ {
				message := makeDropsondeMessage(strconv.Itoa(i), APP_ID, 1234)
		...
		It("returns a multi-part HTTP response with all recent messages", func() {
			...
			Eventually(func() bool {
				messages, err := client.RecentLogs(APP_ID, "bearer iAmAnAdmin")
				Expect(err).NotTo(HaveOccurred())
				select {
				case request := <-fakeDoppler.RecentLogsRequests:
					Expect(request.AppID).To(Equal(APP_ID))
	...
	Context("ContainerMetrics", func() {
		BeforeEach(func() {
			for i := 0; i < 5; i++ {
				message := makeContainerMetricMessage(APP_ID, i, i, i, i, 100000)
				fakeDoppler.SendLogMessage(message)
			}

			oldmessage := makeContainerMetricMessage(APP_ID, 1, 6, 7, 8, 50000)
		...
		It("returns a multi-part HTTP response with the most recent container metrics for all instances for a given app", func() {
			...
			Eventually(func() bool {
				messages, err := client.ContainerMetrics(APP_ID, "bearer iAmAnAdmin")
				Expect(err).NotTo(HaveOccurred())
